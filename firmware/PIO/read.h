...
static inline void count_read_program_init(PIO pio, uint sm, uint offset, uint firstPin, uint countPins, uint triggerPin, uint sampleFreq){
    pio_sm_config c = triggered_read_program_get_default_config(offset);
    sm_config_set_in_pins(&c, firstPin);
...
// static inline void count_read_setSampleLimit(PIO pio, uint sm, uint sampleCount){
//     pio_sm_put_blocking(pio, sm, sampleCount);
// }
